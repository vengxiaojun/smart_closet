...
int aigw_ws_session_update(aigw_ws_ctx_t* ctx, const aigw_ws_session_t* session) {
    ...
    if (session->num_tools > 0) {
        for (int i = 0; i < session->num_tools; i++) {
            cJSON* tool_obj = cJSON_CreateObject();
            cJSON_AddStringToObject(tool_obj, "type", "function");
            cJSON_AddStringToObject(tool_obj, "name", session->tools[i].name);
            cJSON_AddStringToObject(tool_obj, "description", session->tools[i].description);

            // 解析 "parameters" 字符串为 JSON 对象
            cJSON* parameters_obj = cJSON_Parse(session->tools[i].parameters);
...
int aigw_ws_conversation_item_create(aigw_ws_ctx_t* ctx, aigw_ws_conversation_item_create_t *item) {
    if (!ctx || !item) {
        return VOLC_ERR_INVALID_PARAM;
    }
    // 创建根 JSON 对象
    ...
    if(item->function_call_id) {//function result item
        cJSON_AddStringToObject(item_obj, "call_id", item->function_call_id);
        cJSON_AddStringToObject(item_obj, "type", "function_call_output");
        cJSON_AddItemToObject(item_obj, "output", cJSON_Parse(item->output));
    } else {
        // cJSON_AddStringToObject(item_obj, "id", item->id);
        cJSON_AddStringToObject(item_obj, "type", "message");
        cJSON_AddStringToObject(item_obj, "role", "user");
        // 创建 "content" 数组
        cJSON* content_array = cJSON_CreateArray();
        cJSON *content_obj = cJSON_CreateObject();
        cJSON_AddStringToObject(content_obj, "text", item->text);
        cJSON_AddStringToObject(content_obj, "type", "input_text");
        cJSON_AddItemToArray(content_array, content_obj);
        cJSON_AddItemToObject(item_obj, "content", content_array);
    }
    // 生成 JSON 字符串
    const char* json_str = cJSON_PrintUnformatted(root);
    // 释放内存
